...
// ---- Month/Year Popup Picker (uses existing overlay/sheet styles) ----
document.addEventListener('DOMContentLoaded', () => {
  const gm = document.querySelector('#global-month');
  if (!gm) return;

  // Add a small button next to the month input
  let btn = document.querySelector('#month-popup-btn');
  if (!btn) {
    btn = document.createElement('button');
    btn.id = 'month-popup-btn';
    btn.className = 'ghost small';
    btn.type = 'button';
    btn.style.marginLeft = '6px';
    btn.title = 'Pick month';
    btn.textContent = '📅';
    gm.insertAdjacentElement('afterend', btn);
  }

  // Build overlay on demand
  let ov = document.querySelector('#overlay-month');
  if (!ov) {
    ...
    sheet.innerHTML = `
      <h3 style="margin:0 0 8px">Select Month</h3>
      <div class="row" style="justify-content:space-between;margin-bottom:8px">
        <button id="mp-prev" class="ghost small" type="button">◀</button>
        <div id="mp-year" class="pill"></div>
        <button id="mp-next" class="ghost small" type="button">▶</button>
      </div>
      <div id="mp-grid" class="grid three"></div>
      <div class="actions"><button id="mp-cancel" class="ghost">Cancel</button></div>
    `;
  ...
  const yearEl = () => document.querySelector('#mp-year');
  const gridEl = () => document.querySelector('#mp-grid');

  let current = (gm.value && /^\d{4}-\d{2}$/.test(gm.value)) ? new Date(gm.value + '-01') : new Date();
  ...
  function renderGrid() {
    ...
    months.forEach((m, idx) => {
      ...
      b.className = 'pill';
      b.textContent = m;
      b.addEventListener('click', () => chooseMonth(idx));
      grid.appendChild(b);
    });
  }

  function openPicker() {
    // sync current year from input/state
    current = (gm.value && /^\d{4}-\d{2}$/.test(gm.value)) ? new Date(gm.value + '-01') : new Date();
    y = current.getFullYear();
    renderGrid();
    ov.removeAttribute('hidden');
  }
  function closePicker() { ov.setAttribute('hidden',''); }

  function chooseMonth(idx) {
    const mm = String(idx + 1).padStart(2, '0');
    const val = `${y}-${mm}`;
    gm.value = val;
    try { state.month = val; } catch(_) {}
    try { routeRefresh && routeRefresh(); } catch(_) {}
    closePicker();
  }

  btn.addEventListener('click', openPicker);
  ov.addEventListener('click', (e) => { if (e.target === ov) closePicker(); });
  document.querySelector('#mp-cancel')?.addEventListener('click', closePicker);
  document.querySelector('#mp-prev')?.addEventListener('click', () => { y -= 1; renderGrid(); });
  document.querySelector('#mp-next')?.addEventListener('click', () => { y += 1; renderGrid(); });
});
// ---------------------------------------------------------------------

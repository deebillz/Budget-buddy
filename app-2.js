...
function renderCards(){
  ...
  // Cards we track from the Payment field
  const cards = ['Amex Plat','Amex Gold','Amex Blue','Amex Hilton','CSP','CFF','CFU'];

  // Compute from Transactions for selected month
  ...
  rows.forEach(it => {
    ...
    if (it.amount < 0) {
      sums[card] += Math.abs(it.amount);   // expense -> increase card balance
    } else {
      sums[card] -= it.amount;             // income on card -> reduce balance
    }
  });

  // Render table
  const host = $('#card-balances'); if (!host) return;
  const tbl = document.createElement('table');
  tbl.innerHTML = '<thead><tr><th>Card</th><th class="right">Amount</th></tr></thead>';
  ...
  cards.forEach(c => {
    ...
    total += v;
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${escape(c)}</td><td class="right">${fmt(v)}</td>`;
    tb.appendChild(tr);
  });
  const trt = document.createElement('tr');
  trt.innerHTML = `<td><b>Total</b></td><td class="right"><b>${fmt(total)}</b></td>`;
  tb.appendChild(trt);
  tbl.appendChild(tb);
  host.innerHTML = '';
